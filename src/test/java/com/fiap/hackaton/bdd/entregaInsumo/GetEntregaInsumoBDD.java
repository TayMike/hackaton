...
public class GetEntregaInsumoBDD {
    ...
    public void que_foi_construida_uma_entrega_de_insumos() {
        ...
        Response responseEquip2 = given()
                ...
                .post("/entregaInsumos");
        responseEquip2.then().statusCode(HttpStatus.CREATED.value());
        entregaId = responseEquip2.jsonPath().getString("id");
    }

    @Quando("buscar entregas de insumos por hospital - Get")
    public void buscar_entregas_de_insumos_por_hospital() {
        response = given()
                .contentType(MediaType.APPLICATION_JSON_VALUE)
                .when()
                .get("/entregaInsumos/" + entregaId);
    }

    @Então("deve retornar as entregas de insumos do hospital - Get")
    public void deve_retornar_as_entregas_de_insumos_do_hospital() {
